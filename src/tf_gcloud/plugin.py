...
class TerraformGcloudWrapper:
    ...
    def __init__(self):
        ...
        parser.add_argument(
            '-var-file', action='append',
            dest='tfvar_files', help='specify a tfvars file',
        )
        parser.add_argument(
            '-var', action='append',
            dest='inline_vars', help='specify a var',
        )
    ...
    def configure_remotestate(self, required_vars, var_data):
        ...
        self.gcloud_path = run_command.build_tf_state_path(
            self.required_vars, self.var_data,
        )
        self.configure()
        set_remote_backend_status = self.set_remote_backend()
        logger.info(
            'set_remote_backend_status: {}'.format(
    ...
    def set_remote_backend(self):
        ...
        if run_command.build_remote_backend_tf_file('gcs'):
            ...
            if ('backend' in current_tf_state) and (
                    current_tf_state['backend']['config']['bucket'] == self.gcloud_bucket_name
            ) and (
                ...
                cmd = "terraform init -backend-config=\"bucket={}\" -backend-config=\"credentials={}\" " \
                      "-backend-config=\"prefix={}\"".format(
                          self.gcloud_bucket_name, self.gcloud_credentials,
                          gcloud_path,
                      )
